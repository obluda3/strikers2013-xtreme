...
&void displayMovePower() {
  if (Settings.IsMovePowerDisplayed()) {
    ...
    if (!moveType) {
      ...
      sprintf(buffer,
              "#j#=BASE %d#n#oy5#=DIST %d#n#oy5#=RNG %d#n#oy7#z120#=TOTAL %d",
              basePower, distancePenalty, randomBonus, shootPower);
    } else if (moveType == 3) {
      int randomBonus = info.randomBonus;
      int catchPower = info.catchPower;
      int basePower = catchPower - randomBonus;
      sprintf(buffer, "#j#=BASE %d#n#oy5#=RNG %d#n#oy7#z120#=TOTAL %d",
              basePower, randomBonus, catchPower);
    }
