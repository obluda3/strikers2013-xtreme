...
SavePlayerParam *unlockSecretMiximaxes(register PLAYER_DEF *player_def) {
  ...
  bool hasMiximax =
      (player_data->Flag & MIXIMAX_LEVEL_ONE) == MIXIMAX_LEVEL_ONE;
  if (!hasMiximax) {
    for (int i = 0; i < sizeof(MixiUnlockList) / (2*sizeof(int)); i++) {
      if (MixiUnlockList[i * 2] != id)
        continue;
      else {
...
UnlockData Unlocks[] = {
	...
	{P_10442HIKITA, { P_2329FUBUKI, P_2337FUBUKI }, 0, 0, 0},
	{P_10446MUTO, { P_2131ZEL, P_2121DEZARM }, 0, 0, 0},
	{P_12150ZANAKU_EVL, { P_0010GOUENJI_2nd, P_0190AFURO }, 0, 0, 0}
};
u16 ChronoStorm[] = {
    ...
    if (!Savedata_ChkPlayerFlag(recruitedPlayer, UNLOCKED)) {
      for (int j = 0; j < 8; j++) {
        ...
        if (!playerToRecruit) {
          ...
          if (kizunaPlayer1) {
            if (Savedata_getPlayeData_KizunaData(kizunaPlayer1, kizunaPlayer2)
                    ->value < kizunaValue)
              break;
          }
          ...
          break;
        } else if (!Savedata_ChkPlayerFlag(playerToRecruit, RECRUITED))
          break;
      }
    }
  }
