...
void MTX34Scale(MTX34 *dest, const MTX34 *sourceMatrix,
                const VEC3 *sourceVector);
void MTX34Trans(MTX34 *dest, const MTX34 *sourceMatrix,
                const VEC3 *sourceVector);
void MTX34RotAxisFIdx(MTX34 *dest, const VEC3 *vector, float fidx);
void MTX34RotXYZFIdx(MTX34 *dest, float x, float y, float z);
void VEC3TransformNormal(VEC3 *dest, const MTX34 *sourceMatrix,
                         const VEC3 *sourceVector);
...
struct FRUSTUM {
  ...
  void Set(f32 top, f32 bottom, f32 left, f32 right, f32 near, f32 far,
           const MTX34 &matrix);
...
f32 DistSqSegment3ToSegment3(const SEGMENT3 *a, const SEGMENT3 *b, f32 *pOut1,
                             f32 *pOut2);
int IntersectionLine3Sphere(const LINE3 *a, const SPHERE *b, f32 *pOut1,
                            f32 *pOut2);
int IntersectionSegment3Sphere(const SEGMENT3 *a, const SPHERE *b, f32 *pOut1,
                               f32 *pOut2);
...
public:
  ...
  virtual AnimTransform *CreateAnimTransform(const void *data,
                                             ResourceAccessor *resAcc);
  virtual AnimTransform *CreateAnimTransform(const AnimResource &res,
                                             ResourceAccessor *resAcc);

  virtual void BindAnimation(AnimTransform *anim);
  virtual void UnbindAnimation(AnimTransform *anim);
  virtual void UnbindAllAnimation();
  virtual bool BindAnimationAuto(const AnimResource &res,
                                 ResourceAccessor *resAcc);
...
public:
  ...
  float scaleY;
  float alpha;
  u8 _50; // this is actually a bitfield. todo: investigate how CW handles
          // bitfields, and so on
};
...
public:
  ...
  virtual void SetAnimationEnable(const AnimResource &anim, bool unk1,
                                  bool unk2);
...
public:
  TextBox(void *,
          void *); // todo: TextBox((res::TextBox const *,ResBlockSet const &))
  ~TextBox();
...
public:
  Picture(void *,
          void *); // todo: Picture((res::Picture const *,ResBlockSet const &))
  ~Picture();
